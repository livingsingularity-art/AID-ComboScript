...
// ═══════════════════════════════════════════════════════════════════════════════
// ComboScript v1.0 — SharedLibrary
// Unified AI Dungeon scripting engine integrating:
//   NGO (Narrative Gravity Observatory) — heat/temperature tracking
//   BonepokeOS — VSL tension protocol (E/β/LSC), archetypes, correction loop
//   Verbalized Sampling — anti-mode-collapse via probability-distribution prompting
//   Narrative Steering Wheel — player-directed memory injection with TTL
//   JADE Orchestrator — preset mode system for holistic tuning
//
// INSTALLATION: Paste this into your Library tab AFTER InnerSelf/AutoCards code.
// The three hooks (combo_input.js, combo_context.js, combo_output.js) call into
// the modules defined here.
//
// References:
//   AID_Practical_Code_Review (canonical API behavior)
//   AID_Gap_Analysis_Reference (corrections & errata)
//   AID_Integrated_Blueprint (architecture)
// ═══════════════════════════════════════════════════════════════════════════════

// ─────────────────────────────────────────────────────────────────────────────
// SECTION 1: BONEPOKE OS — COMPLETE TENSION ANALYSIS ENGINE
// Faithful port of ProjectBonepoke435.py + JADE VSL coordinates.
// Exposed as globalThis.BonepokeOS for all hooks to call.
// ─────────────────────────────────────────────────────────────────────────────
...
globalThis.BonepokeOS = (function() {
  ...
  var LEXICON = {
    ...
    temporal: [
      ...
      'while','when','formerly','previously',
      'remembered','forgot','returned',
    ...
    sycophancy: [
      ...
      'naturally','obviously','clearly','indeed',
      'undoubtedly','surely','correct','brilliant',
      'fantastic','excellent','remarkable','outstanding',
    ...
    tone: {
      ...
      drop: [
        ...
        'resign','release','withdraw','retreat','soften',
  ...
  // ── 2. THRESHOLDS & CONSTRAINTS ────────────────────────────────────────

  var THRESHOLDS = {
    fatigueThreshold: 3,
    shimmerLimit: 25,
    shimmerMin: 0,
    motifExhaustionThreshold: 3,
    ruptureCooldown: 5,
    minFragmentLength: 5,
    marmActive: 3,
    marmFlicker: 2,
    contradictionBonus: 0.20,
    driftTensionCap: 0.30,
    inversionBonusCap: 0.20,
    symbolismAmpCap: 0.15,
    shearSpike: 0.10,
    shearSpikeThreshold: 2,
    eRepetitionWeight: 0.45,
    eSlopWeight: 0.30,
    eSymbolismWeight: 0.25,
    goldBetaMax: 0.3,
    goldEMax: 0.2,
    slopEMin: 0.5,
    slopBetaMin: 0.5,
    ...
    memoryResidueMax: 20,
    historyMax: 50,
    shimmerResetInterval: 25,
    motifCleanupInterval: 50
  };
  ...
  var CoreEngine = {
    detectContradictions: function(fragment) {
      ...
      for (var i = 0; i < lines.length; i++) {
        ...
        var hasNeg = false, hasTemporal = false;
        ...
        if (hasNeg && hasTemporal) found.push(lines[i]);
      }
      return found;
    },
    ...
    traceFatigue: function(fragment, threshold) {
      ...
      for (var i = 0; i < words.length; i++) {
        ...
        if (w.length < 3 || STOPWORDS.indexOf(w) !== -1) continue;
        counts[w] = (counts[w] || 0) + 1;
      }
      for (var word in counts) {
        if (counts[word] >= threshold) fatigued[word] = counts[word];
      }
      return fatigued;
    },
    ...
    compostDrift: function(fragment) {
      ...
      for (var i = 0; i < lines.length; i++) {
        var line = lines[i].toLowerCase();
        var hasAbstract = false, hasAction = false;
        ...
        if (hasAbstract && !hasAction) drifting.push(lines[i]);
      }
      return drifting;
    },
    ...
    flickerMarm: function(fragment, contradictions, fatigue, drift) {
      ...
      if (score >= THRESHOLDS.marmActive) return 'active';
      if (score >= THRESHOLDS.marmFlicker) return 'flicker';
      return 'suppressed';
    },

    detectSlop: function(fragment) {
      var lower = fragment.toLowerCase();
      var slopHits = countListHits(lower, LEXICON.slopIndicators);
      var sycHits = countListHits(lower, LEXICON.sycophancy);
      return { slopHits: slopHits, sycophancyHits: sycHits, total: slopHits + sycHits };
    },
  ...
  var VSL = {
    calculateE: function(fatigue, slopResult, symbolism, wordCount) {
      ...
      var e = (repetitionRatio * THRESHOLDS.eRepetitionWeight) +
              (slopDensity * THRESHOLDS.eSlopWeight) +
              (symbolismDeficit * THRESHOLDS.eSymbolismWeight);
      return clamp(e, 0, 1);
    },
    ...
    calculateBeta: function(contradictions, drift, toneScores, symbolism, lineCount) {
      ...
      var driftTension = clamp(drift.length * 0.10, 0, THRESHOLDS.driftTensionCap);
      var inversionBonus = clamp(toneScores.invert * 0.08, 0, THRESHOLDS.inversionBonusCap);
      var symAmp = clamp((symbolism.recursion + symbolism.integrity) * 0.05, 0, THRESHOLDS.symbolismAmpCap);
      var beta = contradictionDensity + driftTension + inversionBonus + symAmp;
      if (toneScores.shear > THRESHOLDS.shearSpikeThreshold) beta += THRESHOLDS.shearSpike;
    ...
    calculateLSC: function(fragment, drift) {
      ...
      return clamp((lines.length - drift.length) / lines.length, 0, 1);
    }
  };
  ...
  function determineState(e, beta, lsc) {
    if (beta >= THRESHOLDS.salvageBetaMin && e < THRESHOLDS.salvageEMax && lsc > THRESHOLDS.salvageLSCMin) return 'SALVAGE';
    if (e >= THRESHOLDS.slopEMin || (beta >= THRESHOLDS.slopBetaMin && e >= THRESHOLDS.slopComboE)) return 'SLOP';
    if (beta < THRESHOLDS.goldBetaMax && e < THRESHOLDS.goldEMax) return 'GOLD';
    return 'SALVAGE';
  }
  ...
  function registerShimmerEvent(bp, eventType) {
    bp.shimmerUsed = Math.max(THRESHOLDS.shimmerMin, bp.shimmerUsed || 0);
    var weight = LEXICON.shimmerWeights[eventType] || 1;
    bp.shimmerUsed += weight;
    bp.shimmerUsed = Math.min(bp.shimmerUsed, THRESHOLDS.shimmerLimit * 2);
    return bp.shimmerUsed < THRESHOLDS.shimmerLimit;
  }

  // ── 10. MOTIF DECAY ────────────────────────────────────────────────────

  var TRACKED_MOTIFS = ['loop','ache','echo','shimmer','paradox','grief','ritual','wound'];

  function trackMotifDecay(bp, fragment) {
    bp.motifCounts = bp.motifCounts || {};
    var lower = fragment.toLowerCase();
    for (var i = 0; i < TRACKED_MOTIFS.length; i++) {
      if (lower.indexOf(TRACKED_MOTIFS[i]) !== -1) {
        bp.motifCounts[TRACKED_MOTIFS[i]] = (bp.motifCounts[TRACKED_MOTIFS[i]] || 0) + 1;
      }
    }
    var decayed = {};
    for (var motif in bp.motifCounts) {
      if (bp.motifCounts[motif] > THRESHOLDS.motifExhaustionThreshold) {
  ...
  function checkRuptureCooldown(bp) {
    ...
    bp.tick = Math.max(0, (bp.tick || 0) + 1);
  ...
  // ── 12. CORRECTION SIGNAL ──────────────────────────────────────────────

  function generateCorrection(symbolicState, e, beta) {
    if (symbolicState === 'SALVAGE') {
      return { type: 'MAINTAIN', instruction: '' };
    }
    if (symbolicState === 'GOLD') {
      return {
        type: 'RUPTURE_DEMAND',
        instruction: 'Break cohesion pattern. Introduce structural contradiction. Let opposing forces coexist without premature reconciliation.'
      };
    }
    if (symbolicState === 'SLOP') {
      return {
        type: 'COMPOST_CYCLE',
        instruction: 'Previous output was Slop (E=' + e.toFixed(2) + ', \u03B2=' + beta.toFixed(2) +
          '). Reduce repetition. Eliminate sycophantic agreement. Ground abstractions in concrete action.'
      };
    }
    return { type: 'NONE', instruction: '' };
  }

  // ── 13. PBTestSuite — 11-CATEGORY SCORING ──────────────────────────────

  function runTestSuite(composted) {
    var f = composted.fragment;
    var scores = {};
    function assign(cat, tier, val) { scores[cat] = { tier: tier, score: val }; }
    ...
    assign('World Logic', composted.drift.length > 0 ? 'Salvage' : 'Gold', composted.drift.length > 0 ? 2 : 5);
    ...
    assign('Shimmer Budget', composted.shimmerSafe ? 'Gold' : 'Salvage', composted.shimmerSafe ? 5 : 2);

    var motifCount = Object.keys(composted.motifDecay).length;
    assign('Motif Decay', motifCount === 0 ? 'Gold' : 'Silver', motifCount === 0 ? 5 : 3);

    assign('Rupture Cooldown', composted.ruptureReady ? 'Gold' : 'Slop', composted.ruptureReady ? 5 : 1);
  ...
  function generateSuggestions(composted) {
    ...
    for (var i = 0; i < composted.contradictions.length; i++) {
      suggestions.push("Contradiction: '" + composted.contradictions[i].substring(0, 60) + "' — clarify temporal logic.");
    }
    for (var j = 0; j < composted.drift.length; j++) {
      suggestions.push("Drift: '" + composted.drift[j].substring(0, 60) + "' — add action or decision.");
    }
    for (var word in composted.fatigue) {
      suggestions.push("Repetition: '" + word + "' x" + composted.fatigue[word]);
    }
    if (!composted.shimmerSafe) suggestions.push('Shimmer budget breach — compost rupture or delay recursion.');
    for (var motif in composted.motifDecay) {
      suggestions.push("Motif fatigue: '" + motif + "' x" + composted.motifDecay[motif]);
    }
    if (!composted.ruptureReady) suggestions.push('Rupture cooldown active.');
    return suggestions;
  }
  ...
  function ingest(fragment, bpState, forcedArchetype) {
    ...
    bpState.shimmerUsed = Math.max(THRESHOLDS.shimmerMin, bpState.shimmerUsed || 0);
    ...
    if (fragment.length < THRESHOLDS.minFragmentLength) {
      return { vanillaOk: false, state: 'GOLD', e: 0, beta: 0, lsc: 1, archetype: ARCHETYPES.OBSERVER,
               ...
               symbolism: { earnedness: 0, gravity: 0, recursion: 0, integrity: 0, mythic: 0 },
               shimmerSafe: true, shimmerUsed: bpState.shimmerUsed, shimmerLimit: THRESHOLDS.shimmerLimit,
               motifDecay: {}, ruptureReady: true, testScores: {}, suggestions: [] };
    }
    ...
    var shimmerSafe = registerShimmerEvent(bpState, 'shimmer');
    ...
    var archetype = selectArchetype(e, beta, forcedArchetype);
    var correction = generateCorrection(symbolicState, e, beta);

    var composted = {
      fragment: fragment, contradictions: contradictions, fatigue: fatigue,
      drift: drift, marm: marm, shimmerSafe: shimmerSafe,
      motifDecay: motifDecay, ruptureReady: ruptureReady,
    ...
    for (var r = 0; r < LEXICON.resonance.length; r++) {
      if (lower.indexOf(LEXICON.resonance[r]) !== -1) {
        ...
        if (bpState.memory.length > THRESHOLDS.memoryResidueMax) bpState.memory.shift();
        break;
      }
    }

    bpState.history.push({ e: e, beta: beta, state: symbolicState, tick: bpState.tick });
    if (bpState.history.length > THRESHOLDS.historyMax) bpState.history.shift();
    ...
    return {
      ...
      shimmerUsed: bpState.shimmerUsed, shimmerLimit: THRESHOLDS.shimmerLimit,
      motifDecay: motifDecay, ruptureReady: ruptureReady,
      testScores: testScores, suggestions: suggestions
    };
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// SECTION 2: COMBOENGINE — UNIFIED INTEGRATION ENGINE
// Connects NGO, BonepokeOS, VS, Steering Wheel, and JADE into one pipeline.
// Exposed as globalThis.ComboEngine for all hooks to call.
// ─────────────────────────────────────────────────────────────────────────────

globalThis.ComboEngine = (function() {
  'use strict';

  // ── 1. DEEP MERGE (from Practical Code Review §5.2) ────────────────────
  ...
  // ── 2. UNIFIED VOCABULARY (single-pass scanner) ────────────────────────

  var VOCAB = {
    conflict: ['attack','stab','destroy','break','steal','ruin','burn','smash','sabotage','shoot','punch','hit','slap','scream','yell','gun','pistol','rifle','bullet','ammo','shotgun','firearm','blast','kill','slay','blood','bleed','bleeding','gore','wound','pain','dying','die','perish','rage','anger','betray','revenge','drug','cocaine','heroin','needle','scam','theft','crime','felony','arrest','handcuffs','police','cop','murder','execution','deadly'],
    calming: ['calm','rest','relax','meditate','sleep','comfort','hug','smile','forgive','mend','repair','protect','shelter','trust','hope','eat','drink','balance','laugh','apologize','befriend','thank','appreciate','love','pet','respect','restore','heal','help','assist','romance','affection','flirt','cuddle','care','gentle','soft','tender','kiss','reassure'],
    lift: ['threat','danger','warning','risk','stakes','pressure','demand','confront','challenge','defy','resist','advance','escalate','intensify','sharpen','tighten','narrow','corner','trap','hunt','pursue','stalk','loom','approach','charge','attack','strike','clash','surge','erupt'],
    drop: ['sigh','exhale','ease','settle','relax','calm','quiet','rest','pause','breathe','remember','reflect','mourn','grieve','accept','resign','release','withdraw','retreat','soften','gentle','slow','fade','dim','drift','sink','dissolve','melt'],
    shear: ['crash','shatter','snap','crack','explode','burst','tear','rip','break','smash','collapse','crumble','split','fracture','rupture','scream','thunder','shock','jolt','slam','impact','blast','detonate','pierce','shred','wreck'],
    invert: ['but','however','yet','instead','rather','actually','except','although','despite','nevertheless','regardless','contrary','opposite','reverse','betray','lie','deceive','trick','false','wrong','mistake','illusion','pretend','mask','hidden','secret','twist','turn','reveal','uncover'],
    combat: ['attack','fight','battle','sword','weapon','strike','parry','dodge','shield','armor','arrow','duel'],
    social: ['talk','conversation','persuade','negotiate','charm','argue','discuss','diplomacy','bargain','plead'],
    exploration: ['explore','search','investigate','examine','discover','travel','journey','wander','scout','survey'],
    mystery: ['clue','mystery','suspicious','evidence','detective','secret','hidden','enigma','puzzle','riddle']
  };
  ...
  for (var cat in VOCAB) {
    VOCAB_SETS[cat] = {};
    for (var vi = 0; vi < VOCAB[cat].length; vi++) {
      VOCAB_SETS[cat][VOCAB[cat][vi]] = true;
    }
  }

  var TONE_CATEGORIES = ['lift', 'drop', 'shear', 'invert'];
  var GENRE_CATEGORIES = ['combat', 'social', 'exploration', 'mystery'];
  var GENRE_MIN_HITS = 2;
  ...
  function scanText(text) {
    ...
    for (var i = 0; i < words.length; i++) {
      ...
      if (!w) continue;
      for (var c in VOCAB_SETS) {
        if (VOCAB_SETS[c][w]) result[c]++;
      }
    }
    var maxTone = 0;
    for (var ti = 0; ti < TONE_CATEGORIES.length; ti++) {
      var t = TONE_CATEGORIES[ti];
      if (result[t] > maxTone) { maxTone = result[t]; result.dominantTone = t; }
    }
    var maxGenre = 0;
    for (var gi = 0; gi < GENRE_CATEGORIES.length; gi++) {
      var g = GENRE_CATEGORIES[gi];
      if (result[g] > maxGenre && result[g] >= GENRE_MIN_HITS) { maxGenre = result[g]; result.dominantGenre = g; }
    }
    return result;
  }

  // ── 3. CONSTRAINTS (min/max for every tunable value) ───────────────────

  var CONSTRAINTS = {
    heat:        { min: 0,    max: 100 },
    temperature: { min: 1,    max: 15  },
    vsThreshold: { min: 0.01, max: 1.0 },
    vsCandidates:{ min: 3,    max: 10  },
    steerTTL:    { min: 1,    max: 20  },
    steerSlots:  3,
    maxFingerprints: 10,
    maxFingerprintWords: 20,
    collapseHighThreshold: 0.6,
    collapseMidThreshold: 0.4
  };
  ...
  var JADE_MODES = {
    GRITTY: {
      ...
      vsThreshold: 0.15, vsCandidates: 5, vsEnabled: true, bpEnabled: true, steerEnabled: true,
      baseNote: 'Gritty, chaotic, morally gray. Relationships carry tension, loyalty, secrets, desire, betrayal. Small actions matter: silence, looks, slammed doors. Humor mixes with danger. Violence is realistic. Follow through with consequences. Mature themes appear with realism, not glamorization.',
      heatFloor: 3, correctionDisabled: false
    },
    WHIMSICAL: {
      desc: 'Lighter tone — surprise over tension',
      vsThreshold: 0.08, vsCandidates: 7, vsEnabled: true, bpEnabled: true, steerEnabled: true,
      baseNote: 'Playful, unexpected, wonder-filled. Characters are quirky and endearing. Surprises are delightful rather than threatening. Humor is warm. Stakes exist but the world rewards curiosity.',
      heatFloor: 0, correctionDisabled: false
    },
    LITERARY: {
      desc: 'Maximum structural tension — Bonepoke in overdrive',
      vsThreshold: 0.10, vsCandidates: 6, vsEnabled: true, bpEnabled: true, steerEnabled: true,
      baseNote: 'Dense, layered prose. Every sentence earns its place. Subtext matters more than text. Symbolism is earned through action, not decoration. Let silence carry weight.',
      heatFloor: 0, correctionDisabled: false
    },
    FREEFORM: {
      desc: 'All analysis on, no correction — instrument panel only',
      vsThreshold: 0.20, vsCandidates: 5, vsEnabled: true, bpEnabled: true, steerEnabled: true,
      baseNote: '',
      heatFloor: 0, correctionDisabled: true
    },
    RAW: {
      desc: 'Everything off — vanilla AID experience',
      vsThreshold: 0.20, vsCandidates: 5, vsEnabled: false, bpEnabled: false, steerEnabled: false,
      baseNote: '',
      heatFloor: 0, correctionDisabled: true
    }
  };
  ...
  function initState() {
    state.combo = deepMerge(state.combo || {}, {
      ...
      baseAuthorsNote: 'Gritty, chaotic, morally gray. Relationships carry tension, loyalty, secrets, desire, betrayal. Small actions matter: silence, looks, slammed doors. Humor mixes with danger. Violence is realistic. Follow through with consequences. Mature themes appear with realism, not glamorization.',
      inputSnapshot: {
        tone: { lift: 0, drop: 0, shear: 0, invert: 0, dominant: 'neutral', intensity: 0 },
        genre: null,
        conflictCount: 0,
        calmingCount: 0
      },
  ...
  function dbg(label, val) {
    if (state.combo && state.combo.debug) {
      var out = '[COMBO:' + label + '] ';
      if (val === undefined) {
        out += 'undefined';
      } else if (val === null) {
  ...
  function clampHeat(combo) {
    combo.heat = Math.max(combo.config.minHeat, Math.min(combo.config.maxHeat, combo.heat));
  }

  function clampTemperature(combo) {
    combo.temperature = Math.max(combo.config.minTemperature, Math.min(combo.config.maxTemperature, combo.temperature));
  }

  function clampVS(combo) {
    combo.vs.threshold = Math.max(CONSTRAINTS.vsThreshold.min, Math.min(CONSTRAINTS.vsThreshold.max, combo.vs.threshold));
    combo.vs.candidates = Math.max(CONSTRAINTS.vsCandidates.min, Math.min(CONSTRAINTS.vsCandidates.max, combo.vs.candidates));
  }

  // ── 8. VS FINGERPRINT & COLLAPSE DETECTION ────────────────────────────

  function fingerprint(text) {
    var words = text.toLowerCase().replace(/[^\w\s]/g, '').split(/\s+/).filter(function(w) { return w.length > 3; });
    var freq = {};
    for (var i = 0; i < words.length; i++) freq[words[i]] = (freq[words[i]] || 0) + 1;
    var entries = [];
    for (var w in freq) entries.push([w, freq[w]]);
    entries.sort(function(a, b) { return b[1] - a[1]; });
    var top = [];
    for (var j = 0; j < Math.min(entries.length, CONSTRAINTS.maxFingerprintWords); j++) {
      top.push(entries[j][0]);
    }
    return top;
  }
  ...
  function jaccardSimilarity(a, b) {
    ...
    var aSize = Object.keys(setA).length;
    var bSize = Object.keys(setB).length;
    var union = aSize + bSize - intersection;
    return union > 0 ? intersection / union : 0;
  }

  function measureCollapse(currentFP, fingerprints) {
    if (fingerprints.length < 2) return 0;
    var total = 0;
    for (var i = 0; i < fingerprints.length; i++) {
      total += jaccardSimilarity(currentFP, fingerprints[i]);
    }
    return total / fingerprints.length;
  }

  function recordFingerprint(combo, text) {
    var fp = fingerprint(text);
    combo.vs.fingerprints.push(fp);
    if (combo.vs.fingerprints.length > CONSTRAINTS.maxFingerprints) {
      combo.vs.fingerprints = combo.vs.fingerprints.slice(-CONSTRAINTS.maxFingerprints);
    }
    var collapse = measureCollapse(fp, combo.vs.fingerprints);
    combo.vs.collapseScore = Math.round(collapse * 100) / 100;
    return combo.vs.collapseScore;
  }

  // ── 9. ARBITER — UNIFIED DECISION ENGINE ───────────────────────────────

  function arbitrate(combo) {
    var bp = combo.bp.lastAnalysis;
    var vs = combo.vs;
    var input = combo.inputSnapshot;
    var steer = combo.steer;
    ...
    // ── Archetype from BonepokeOS ─────────────────────────────────────
    if (combo.bp.enabled && bp && bp.vanillaOk) {
      decision.archetype = bp.archetype;
      decision.archetypeNote = bp.archetype.note;
      dbg('arbiter.archetype', bp.archetype.id);
    }

    // ── VS adaptation driven by BP state ──────────────────────────────
    if (combo.bp.enabled && bp && bp.vanillaOk && vs.enabled) {
      if (bp.e > 0.4 && vs.collapseScore > CONSTRAINTS.collapseMidThreshold) {
        decision.vsMode = 'tail';
        decision.vsThreshold = Math.max(CONSTRAINTS.vsThreshold.min, vs.threshold * 0.6);
        decision.vsCandidates = 7;
        dbg('arbiter.vs', 'aggressive tail — high E + high collapse');
      } else if (bp.state === 'GOLD') {
        decision.vsMode = 'tail';
        decision.vsThreshold = 0.10;
        dbg('arbiter.vs', 'tail — GOLD cohesion trap');
      } else if (bp.state === 'SALVAGE') {
        decision.vsMode = 'standard';
        decision.vsThreshold = 0.20;
        dbg('arbiter.vs', 'standard — SALVAGE maintained');
      } else if (bp.state === 'SLOP') {
        decision.vsMode = 'tail';
        decision.vsThreshold = 0.08;
        decision.vsCandidates = 6;
        dbg('arbiter.vs', 'aggressive tail — SLOP detected');
      }
    }

    // ── Genre override for VS ─────────────────────────────────────────
    if (input.genre && vs.enabled) {
      var genreThresholds = { combat: 0.15, social: 0.10, exploration: 0.08, mystery: 0.12 };
      if (genreThresholds[input.genre]) {
        decision.vsThreshold = genreThresholds[input.genre];
        decision.vsMode = 'tail';
        dbg('arbiter.genre', input.genre + ' -> threshold=' + decision.vsThreshold);
      }
    }

    // ── Clamp VS values ───────────────────────────────────────────────
    decision.vsThreshold = Math.max(CONSTRAINTS.vsThreshold.min, Math.min(CONSTRAINTS.vsThreshold.max, decision.vsThreshold));
    decision.vsCandidates = Math.max(CONSTRAINTS.vsCandidates.min, Math.min(CONSTRAINTS.vsCandidates.max, decision.vsCandidates));

    // ── Correction signal ─────────────────────────────────────────────
    if (!combo.correctionDisabled && combo.bp.correctionActive && combo.bp.correctionSignal) {
      decision.correction = combo.bp.correctionSignal;
      dbg('arbiter.correction', decision.correction.substring(0, 60));
    }
    ...
    if (input.tone && input.tone.dominant !== 'neutral') {
      decision.toneGuidance = toneMap[input.tone.dominant] || '';
      if (input.tone.dominant === 'invert' && input.tone.invert > 1) {
        decision.correction = '';
        dbg('arbiter.tone', 'inversion cleared correction');
      }
    }

    // ── Tone-archetype reinforcement ──────────────────────────────────
    if (decision.archetype && input.tone) {
      ...
      if (input.tone.dominant === 'lift' && decision.archetype.id === 'CALCIFIER') {
        decision.archetypeNote += ' The player is escalating into chaos — crystallize the volatile material now.';
      }
    }

    // ── Steering Wheel memories ───────────────────────────────────────
    if (steer.enabled) {
      var activeMemories = steer.memories.filter(function(m) { return m && m.length > 0; });
      if (activeMemories.length > 0) {
        var anchor = steer.lastInputWords || steer.lastOutputWords || '';
        if (anchor) {
          decision.steerTransition = 'Immediately after "' + anchor + '", transition toward: ' + activeMemories.join(', ') + '.';
        } else {
          decision.steerTransition = 'Weave the following themes into the narrative: ' + activeMemories.join(', ') + '.';
        }
        dbg('arbiter.steer', decision.steerTransition.substring(0, 60));
      }
    }

    // ── Heat description ──────────────────────────────────────────────
    var h = combo.heat;
    if (h <= 3) decision.heatDescription = 'Calm, tension dormant.';
    else if (h <= 8) decision.heatDescription = 'Underlying tension simmers.';
    else if (h <= 15) decision.heatDescription = 'Volatile. Conflict is close.';
    else decision.heatDescription = 'Imminent eruption.';

    // ── Motif decay warnings ──────────────────────────────────────────
    if (bp && bp.motifDecay) {
      var exhausted = Object.keys(bp.motifDecay);
      if (exhausted.length > 0) {
        decision.motifWarnings = exhausted;
        dbg('arbiter.motifs', exhausted.join(','));
      }
    }

    // ── MARM canary ───────────────────────────────────────────────────
    if (bp && bp.marm === 'active') {
      decision.marmNote = 'MARM active: the narrative is in a charged state. A breakthrough or collapse is imminent.';
    }

    // ── Build VS instruction ──────────────────────────────────────────
    if (vs.enabled && decision.vsMode !== 'off') {
      var k = decision.vsCandidates;
      var p = decision.vsThreshold;
      if (decision.vsMode === 'tail') {
        decision.vsInstruction = '[Internal process: Before writing, silently consider ' + k +
          ' distinct continuations from the probability tails (p < ' + p.toFixed(2) +
          '). Select one. Output only the selected continuation.]';
      } else {
        decision.vsInstruction = '[Internal process: Before writing, silently consider ' + k +
          ' distinct continuations spanning the full probability distribution. Estimate relative' +
          ' probabilities. Select one weighted by probability. Output only the selected continuation.]';
      }
      dbg('arbiter.vs.instruction', decision.vsMode + '@' + p.toFixed(2) + ' k=' + k);
    }

    return decision;
  }

  // ── 10. AUTHORS NOTE BUILDER ───────────────────────────────────────────

  function buildAuthorsNote(combo, decision) {
    var layers = [];

    // Layer 1: Scenario base note (highest priority)
    if (combo.baseAuthorsNote) {
      layers.push(combo.baseAuthorsNote);
    }

    // Layer 2: Steering Wheel transition
    if (decision.steerTransition) {
      layers.push(decision.steerTransition);
    }

    // Layer 3: VSL state + archetype mandate
    var bp = combo.bp.lastAnalysis;
    if (combo.bp.enabled && bp && bp.vanillaOk && decision.archetype) {
      layers.push('[VSL E:' + bp.e.toFixed(2) + ' \u03B2:' + bp.beta.toFixed(2) +
        ' LSC:' + bp.lsc.toFixed(2) + ' | ' + bp.state + ']' +
        ' [' + decision.archetype.id + ': ' + decision.archetype.mandate + ']');
      layers.push(decision.archetypeNote);
    }

    // Layer 4: Tone guidance
    if (decision.toneGuidance) {
      layers.push(decision.toneGuidance);
    }

    // Layer 5: Heat description
    layers.push('[HEAT:' + combo.heat + ' TEMP:' + combo.temperature + '] ' + decision.heatDescription);

    // Layer 6: Correction signal
    if (decision.correction) {
      layers.push('[CORRECTION] ' + decision.correction);
    }

    // Layer 7: Motif decay warnings
    if (decision.motifWarnings.length > 0) {
      layers.push('Avoid overusing: ' + decision.motifWarnings.join(', ') + '. Find fresh language.');
    }

    return layers.join('\n');
  }

  // ── 11. FRONT MEMORY BUILDER ───────────────────────────────────────────

  function buildFrontMemory(combo, decision) {
    var parts = [];

    // Part 1: VS selection constraint (never reduced — most critical)
    if (decision.vsInstruction) {
      parts.push(decision.vsInstruction);
    }

    // Part 2: D20 roll result
    if (combo.roll.frontMemory && combo.roll.action === info.actionCount) {
      parts.push(combo.roll.frontMemory);
    }

    // Part 3: MARM canary
    if (decision.marmNote) {
      parts.push(decision.marmNote);
    }

    return parts.join(' ');
  }

  // ── 12. DASHBOARD FORMATTER ────────────────────────────────────────────

  function formatDashboard(combo) {
    var parts = [];
    var bp = combo.bp.lastAnalysis;

    if (combo.bp.enabled && bp && bp.vanillaOk) {
      parts.push('E:' + bp.e.toFixed(2));
      parts.push('\u03B2:' + bp.beta.toFixed(2));
      parts.push(bp.state);
      parts.push(bp.archetype.id);
      parts.push('MARM:' + bp.marm);
      parts.push('Shim:' + bp.shimmerUsed + '/' + bp.shimmerLimit);
    } else {
      parts.push('BP:off');
    }

    if (combo.vs.enabled) {
      parts.push('VS:' + combo.vs.mode + '@' + combo.vs.threshold.toFixed(2));
    } else {
      parts.push('VS:off');
    }

    parts.push('HEAT:' + combo.heat);
    parts.push('TEMP:' + combo.temperature);
    parts.push('Turn:' + info.actionCount);

    if (combo.steer.enabled) {
      var activeCount = combo.steer.memories.filter(function(m) { return m && m.length > 0; }).length;
      if (activeCount > 0) parts.push('Steer:' + activeCount);
    }
  ...
  function ensureCard(title, entry, type) {
    type = type || 'system';
    var card = null;
  ...
  function applyMode(modeName) {
    var mode = JADE_MODES[modeName];
    if (!mode) return false;
    var combo = state.combo;
    combo.mode = modeName;
    combo.vs.enabled = mode.vsEnabled;
    combo.vs.threshold = mode.vsThreshold;
    combo.vs.candidates = mode.vsCandidates;
    combo.bp.enabled = mode.bpEnabled;
    combo.steer.enabled = mode.steerEnabled;
    combo.baseAuthorsNote = mode.baseNote;
    combo.config.minHeat = mode.heatFloor;
    combo.correctionDisabled = mode.correctionDisabled;
    clampHeat(combo);
    clampTemperature(combo);
    clampVS(combo);
    dbg('mode.applied', modeName);
    return true;
  }

  // ── 15. STATUS REPORT ──────────────────────────────────────────────────

  function getStatusReport(combo) {
    var lines = [];
    lines.push('=== ComboScript v1.0 Status ===');
    lines.push('Mode: ' + (combo.mode || 'GRITTY'));
    lines.push('Master: ' + (combo.enabled ? 'ON' : 'OFF'));
    lines.push('Debug: ' + (combo.debug ? 'ON' : 'OFF'));
    lines.push('--- NGO ---');
    lines.push('Heat: ' + combo.heat + ' [' + combo.config.minHeat + '-' + combo.config.maxHeat + ']');
    lines.push('Temp: ' + combo.temperature + ' [' + combo.config.minTemperature + '-' + combo.config.maxTemperature + ']');
    lines.push('--- BonepokeOS ---');
    lines.push('BP: ' + (combo.bp.enabled ? 'ON' : 'OFF'));
    var bp = combo.bp.lastAnalysis;
    if (bp && bp.vanillaOk) {
      lines.push('E: ' + bp.e.toFixed(2) + ' | \u03B2: ' + bp.beta.toFixed(2) + ' | LSC: ' + bp.lsc.toFixed(2));
      lines.push('State: ' + bp.state + ' | Archetype: ' + bp.archetype.id);
      lines.push('MARM: ' + bp.marm);
      lines.push('Shimmer: ' + bp.shimmerUsed + '/' + bp.shimmerLimit);
      lines.push('Correction: ' + (combo.bp.correctionActive ? combo.bp.correctionSignal.substring(0, 50) : 'none'));
      var exhausted = bp.motifDecay ? Object.keys(bp.motifDecay) : [];
      lines.push('Exhausted motifs: ' + (exhausted.length > 0 ? exhausted.join(', ') : 'none'));
    } else {
      lines.push('(no analysis yet — play a turn first)');
    }
    lines.push('Forced archetype: ' + (combo.bp.forcedArchetype || 'none'));
    lines.push('--- Verbalized Sampling ---');
    lines.push('VS: ' + (combo.vs.enabled ? 'ON' : 'OFF'));
    lines.push('Mode: ' + combo.vs.mode + ' | Threshold: ' + combo.vs.threshold.toFixed(2) + ' | Candidates: ' + combo.vs.candidates);
    lines.push('Collapse score: ' + combo.vs.collapseScore.toFixed(2));
    lines.push('Fingerprints stored: ' + combo.vs.fingerprints.length + '/' + CONSTRAINTS.maxFingerprints);
    lines.push('--- Steering Wheel ---');
    lines.push('Steer: ' + (combo.steer.enabled ? 'ON' : 'OFF'));
    for (var si = 0; si < combo.steer.memories.length; si++) {
      var mem = combo.steer.memories[si];
      var exp = combo.steer.expirations[si];
      if (mem) {
        lines.push('  Slot ' + (si + 1) + ': "' + mem + '" (expires turn ' + exp + ')');
      } else {
        lines.push('  Slot ' + (si + 1) + ': empty');
      }
    }
    lines.push('--- D20 ---');
    lines.push('Last roll action: ' + combo.roll.action + ' | Result: ' + (combo.roll.frontMemory || 'none'));
    lines.push('--- Turn: ' + info.actionCount + ' ---');
    return lines.join('\n');
  }
  ...
  return {
    ...
    scanText: scanText,
    arbitrate: arbitrate,
    buildAuthorsNote: buildAuthorsNote,
    buildFrontMemory: buildFrontMemory,
    ...
    JADE_MODES: JADE_MODES,
    VOCAB: VOCAB
  };
})();

// ── INITIALIZE STATE ON LIBRARY LOAD ─────────────────────────────────────
// SharedLibrary re-executes before every hook (Practical Code Review §1).
// deepMerge ensures existing state values are preserved, only missing keys
// are filled with defaults.
ComboEngine.initState();
